...
io.sockets.on('connection', socket => {
  ...
  socket.on('JOIN', ({ room_id, name }) => {
    console.log(room_id);
    if(rooms[room_id] && !rooms[room_id].hasStarted) {
      socket.join(room_id);

      console.log('rooms: ' + rooms);
      console.log('room_id: ' + room_id);
      console.log('on join log: ' + rooms.room_id);

      const currentParticipants = rooms[room_id].participants;
      currentParticipants.push({ id: socket.id, name: name, score: 0, hasSelected: false });

      // rooms[room_id] = { ...rooms[room_id], participants: currentParticipants };

      io.sockets.in(room_id).emit('ROOM_INFO', { room_id: room_id, room: rooms[room_id] });
      socket.emit('JOIN_SUCCESS', room_id);
    }
    else socket.emit('JOIN_ERROR', room_id);
  });

  socket.on('START_GAME', (room_id) => {
    rooms[room_id] = { ...rooms[room_id], hasStarted: true };
    io.sockets.in(room_id).emit('ROOM_INFO', { room_id: room_id, room: rooms[room_id] });
  });
